...
describe('AppSidebar', () => {
  ...
  const rolesSignal = signal<readonly string[]>([]);
  ...
  it('shows all clinical navigation items for a clinician', () => {
    ...
    expect(element.textContent).toContain('Appointments');
  ...
  it('hides Clinician-only appointments and contextual Clinical Notes for a referral coordinator', () => {
    ...
    expect(element.textContent).not.toContain('Appointments');

    expect(element.textContent).not.toContain('Clinical Notes');
  });

  it('shows only unrestricted navigation for an administrator-only user', () => {
    rolesSignal.set([CARETRACK_ROLES.administrator]);
    ...
    expect(element.textContent).not.toContain('Referrals');
  ...
  it('emits a collapse request when the toggle is clicked', () => {
    const emitSpy = vi.spyOn(component.collapseToggle, 'emit');
  ...
  it('emits navigationSelected when a nav item is clicked', async () => {
    const emitSpy = vi.spyOn(component.navigationSelected, 'emit');
  ...
  it('shows branding by default', () => {
    ...
    expect(element.textContent).toContain('Clinical operations');
  });

  it('hides branding when showBrand is false', () => {
    fixture.componentRef.setInput('showBrand', false);

    fixture.detectChanges();

    const element = fixture.nativeElement as HTMLElement;

    expect(element.textContent).not.toContain('Clinical operations');
  });

  it('hides the collapse control when requested', () => {
    fixture.componentRef.setInput('showCollapseControl', false);
