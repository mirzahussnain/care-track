import { msalInterceptorConfigurationFactory } from './auth.config';
...
describe('msalInterceptorConfigurationFactory', () => {
  it('maps CareTrack API routes to the delegated API scope', () => {
    const config = msalInterceptorConfigurationFactory();

    expect(config.protectedResourceMap.get(`${environment.apiBaseUrl}/*`)).toEqual([
